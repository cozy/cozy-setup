...
casper.test.begin 'Cozy need to be registered on first launch', (test) ->
	...
	casper.wait '500', ->
	#casper.waitUntilVisible '#email-input', ->
	#casper.waitForUrl /https:\/\/127.0.0.1\/register$/, ->
		test.assertTitle 'Cozy - Sign up', 'We need to be registered on first launch'
		test.assertExists 'input[id="email-input"]', 'We found email input'
		@echo 'We fill form'
	...
	casper.waitUntilVisible '.alert-success', ->
		test.assertNotVisible '.alert-error', 'We did\'nt found error message'
		test.assertVisible '.alert-success', 'We have success message show'
		test.assertEval ->
